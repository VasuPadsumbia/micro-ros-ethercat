...
gen_slave_config() {
    # Convert config.in EC_* vars → ethercat-slave/rtl/slave_config.vh
    local cfg="$SLAVE_DIR/rtl/slave_config.vh"
    info "Generating slave_config.vh from config.in..."
    cat > "$cfg" <<VERILOG
// AUTO-GENERATED by run.sh from config.in — do not edit directly.
// To change: edit config.in, then run: ./run.sh --build slave
\`ifndef SLAVE_CONFIG_VH
\`define SLAVE_CONFIG_VH
\`define PHY0_ADDR       5'd${EC_PHY_ADDR:-1}
\`define EC_STATION_ADDR 16'd${EC_STATION_ADDR:-1}
\`define MBX_OUT_BASE    16'h$(printf '%04X' "${EC_MBX_OUT_BASE:-4096}")
\`define MBX_OUT_SIZE    16'd${EC_MBX_OUT_SIZE:-256}
\`define MBX_IN_BASE     16'h$(printf '%04X' "${EC_MBX_IN_BASE:-4352}")
\`define MBX_IN_SIZE     16'd${EC_MBX_IN_SIZE:-256}
\`define EEPROM_I2C_ADDR 7'h$(printf '%02X' "${EC_EEPROM_ADDR:-80}")
\`define MDC_DIV         ${EC_MDC_DIV:-6}
\`define I2C_CLK_DIV     ${EC_I2C_CLK_DIV:-67}
\`endif // SLAVE_CONFIG_VH
VERILOG
    ok "slave_config.vh written"
}
